...
const TwoFactorSettings = () => {
  ...
  // ── Step 2: Verifikasi kode dengan verifyTotp() ── BENAR-BENAR validasi kode
  // verifyTotp() adalah satu-satunya method yang betul-betul cek 6-digit vs secret
  const handleVerifyAndEnable = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (authError) {
        setError(
          "Kode salah atau sudah kadaluarsa. Buka Authenticator dan masukkan kode terbaru."
        );
        setVerifyCode("");
        return;
      }

      // Kode valid → langsung ke dashboard
      ...
      <div className="p-6">
        ...
        {!is2FAEnabled && (
          <>
            ...
            {/* STEP: Scan QR + Backup Codes + Masukkan Kode (semua dalam 1 halaman) */}
            {setupStep === "scan-and-verify" && (
              <form onSubmit={handleVerifyAndEnable} className="space-y-5">
                ...
                {/* Backup Codes — tampilkan sebelum user verifikasi */}
                {backupCodes.length > 0 && (
                  <div className="bg-amber-50 border border-amber-200 rounded-xl p-4">
                    ...
                    <p className="text-[11px] text-amber-700 mb-3">Gunakan jika kehilangan akses ke Authenticator. Setiap kode hanya berlaku <strong>sekali</strong>.</p>
                    <div className="grid grid-cols-2 gap-1.5">
                      {backupCodes.map((bc, i) => (
                        ...
                        >
                          ...
                          {copiedIndex === i ? <Check className="w-3 h-3 text-green-500" /> : <Copy className="w-3 h-3 text-amber-300 group-hover:text-amber-600" />}
                        </button>
                      ))}
                    </div>
                  </div>
